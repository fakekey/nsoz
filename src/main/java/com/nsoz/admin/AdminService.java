...
public class AdminService {
    ...
    public void addEquipment(Char p, String[] args) {
        try {
            ...
            if (level >= 90 && level < 100) {
                ...
                if (itemID != -1) {
                    item = ItemFactory.getInstance().newItem9X(itemID, max, (byte) sys2);
                }

            } else {
                ItemStore itemStore = StoreManager.getInstance().getEquipment(level, sys2, gender);
                if (itemStore != null) {
                    item = Converter.getInstance().toItem(itemStore, max ? Converter.MAX_OPTION : Converter.RANDOM_OPTION);
                }
            }
            if (item != null) {
                item.next(upgrade);
    ...
    public void addGem(Char p, String[] args) {
        ...
        if (itemID != ItemName.HUYEN_TINH_NGOC && itemID != ItemName.HUYET_NGOC && itemID != ItemName.LAM_TINH_NGOC && itemID != ItemName.LUC_NGOC) {
    ...
    public void setSkillWithLevel(Char p, int level) {
        ...
        if (skill != null) {
            ...
            p.hp = p.maxHP;
            p.mp = p.maxMP;
            p.getService().updateHp();
            p.getService().updateMp();
            p.tangKyNang6x(p.equipment[ItemTemplate.TYPE_BIKIP], false);
        }
    }
        ...
        if (p.isModeCreate) {
            ...
            if (text.startsWith("addmob")) {
                ...
                if (text.length() > 6) {
                    ...
                    Mob monster = new Mob(id++, (short) mobId, te.hp, te.level, p.x, (short) (p.y - t), false, te.isBoss(), p.zone);
