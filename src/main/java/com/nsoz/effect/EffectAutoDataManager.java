...
public class EffectAutoDataManager {
    ...
    public static final int CAY_THONG_NOEL = 13;
    public static final int CAY_HOA_MAI = 14;
    public static final int CAY_HOA_DAO = 15;
    ...
    public static void readEffectAutoDataFrom(String filePath) {
        byte[] effAutoBytes = NinjaUtils.getFile(filePath);
        ByteArrayInputStream bis = new ByteArrayInputStream(effAutoBytes);
        DataInputStream dis = new DataInputStream(bis);
        EffectAutoTemp effAutoTmp = new EffectAutoTemp();
        try {
            effAutoTmp.imgInfo = new ImageInfo[(int) dis.readByte()];
            for (int i = 0; i < effAutoTmp.imgInfo.length; i++) {
                effAutoTmp.imgInfo[i] = new ImageInfo();
                effAutoTmp.imgInfo[i].id = (int) dis.readByte();
                effAutoTmp.imgInfo[i].x0 = dis.readUnsignedByte();
                effAutoTmp.imgInfo[i].y0 = dis.readUnsignedByte();
                effAutoTmp.imgInfo[i].w = dis.readUnsignedByte();
                effAutoTmp.imgInfo[i].h = dis.readUnsignedByte();
            }
            effAutoTmp.frameEffAuto = new Frame[(int) dis.readShort()];
            for (int j = 0; j < effAutoTmp.frameEffAuto.length; j++) {
                effAutoTmp.frameEffAuto[j] = new Frame();
                byte b = dis.readByte();
                effAutoTmp.frameEffAuto[j].dx = new int[(int) b];
                effAutoTmp.frameEffAuto[j].dy = new int[(int) b];
                effAutoTmp.frameEffAuto[j].idImg = new int[(int) b];
                for (int k = 0; k < (int) b; k++) {
                    effAutoTmp.frameEffAuto[j].dx[k] = dis.readShort();
                    effAutoTmp.frameEffAuto[j].dy[k] = dis.readShort();
                    effAutoTmp.frameEffAuto[j].idImg[k] = dis.readByte();
                }
            }
            short runningLength = dis.readShort();
            effAutoTmp.frameRunning = new short[(int) runningLength];
            for (int l = 0; l < (int) runningLength; l++) {
                effAutoTmp.frameRunning[l] = dis.readShort();
            }
            bis.close();
            dis.close();
        } catch (Exception e) {
            ...
            Log.info(String.format("%s: %s", filePath, effAutoTmp.toJson()));
        }
    }
