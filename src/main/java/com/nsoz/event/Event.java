...
public abstract class Event {
    ...
    public void loadEventPoint() {
        try {
            ...
            while (rs.next()) {
                ...
                ArrayList<Point> points = g.fromJson(rs.getString("point"), new TypeToken<ArrayList<Point>>() {}.getType());
                eventPoint.setId(id);
    ...
    public boolean makeEventItem(Char p, int number, int[][] itemRequire, int gold, int coin, int yen, RandomCollection<Integer> rc,
            int itemIdReceive) {
        if (number < 1) {
