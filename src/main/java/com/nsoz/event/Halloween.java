...
public class Halloween extends Event {
    ...
    public void useItem(Char _char, Item item) {
        if (item.id == ItemName.THU_MOI_LE_HOI) {
            ...
            _char.serverMessage("Số lượt tham gia lễ hội hoá trang: " + _char.getEventPoint().find(INVITATION_NUMBER).getPoint());
    ...
    public void makeKey(Char p, int amount) {
        int[][] itemRequires =
                new int[][] {{ItemName.H, 1}, {ItemName.A, 1}, {ItemName.L, 2}, {ItemName.O, 1}, {ItemName.W, 1}, {ItemName.E, 2}, {ItemName.N, 1}};
    ...
    public void menu(Char p) {
        ...
        p.menus.add(new Menu(CMDMenu.EXECUTE, "Hướng dẫn", () -> {
            StringBuilder sb = new StringBuilder();
            sb.append("- Điểm tiêu xài: ").append(NinjaUtils.getCurrency(p.getEventPoint().getPoint(EventPoint.DIEM_TIEU_XAI))).append("\n");
    ...
    public void initStore() {
        StoreManager.getInstance().addItem((byte) StoreManager.TYPE_MISCELLANEOUS,
                ItemStore.builder().id(998).itemID(ItemName.BI_MA).gold(20).expire(ConstTime.FOREVER).build());
        ...
        StoreManager.getInstance().addItem((byte) StoreManager.TYPE_MISCELLANEOUS,
                ItemStore.builder().id(1000).itemID(ItemName.BI_RE_HANH).gold(500).expire(ConstTime.WEEK).options(options).build());
        ...
        StoreManager.getInstance().addItem((byte) StoreManager.TYPE_MISCELLANEOUS,
                ItemStore.builder().id(1001).itemID(ItemName.JACK_HOLLOW).gold(500).expire(ConstTime.WEEK).options(options2).build());

        StoreManager.getInstance().addItem((byte) StoreManager.TYPE_MISCELLANEOUS,
                ItemStore.builder().id(1002).itemID(ItemName.THU_MOI_LE_HOI).gold(20).expire(ConstTime.FOREVER).build());
    }
