...
public class OokazaGymnasium extends Gymnasium {
    ...
    @Override
    public void initBot() {
        Bot bot = Bot.builder().id(-11111).name("Thầy Ookamesama").level(50).typePk(Char.PK_DOSAT).build();
        bot.setDefault();
        FashionCustom fashionCustom = FashionCustom.builder().head((short) 53).body((short) 54).leg((short) 55).weapon((short) -1).build();
        ...
        MoveWithinCustom move = MoveWithinCustom.builder().minX(696).maxX(1176).minY(100).maxY(240).build();
        bot.setMove(move);
