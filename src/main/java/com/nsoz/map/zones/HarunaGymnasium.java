...
public class HarunaGymnasium extends Gymnasium {
    ...
    @Override
    public void initBot() {
        Bot bot = Bot.builder().id(-11111).name("Thầy Kazeto").level(50).typePk(Char.PK_DOSAT).build();
        bot.setDefault();
        FashionCustom fashionCustom = FashionCustom.builder().head((short) 65).body((short) 66).leg((short) 67).weapon((short) -1).build();
        ...
        MoveWithinCustom move = MoveWithinCustom.builder().minX(792).maxX(1176).minY(100).maxY(240).build();
        bot.setMove(move);
