...
public class Zone {
    ...
    public void init() {
        ...
        for (Npc npc : tilemap.npcs) {
            Npc newNpc = NpcFactory.getInstance().newNpc(npc.id, npc.template.npcTemplateId, npc.cx, npc.cy, npc.status);
            newNpc.setService(service);
            addNpc(newNpc);
        }

        if (Event.isEvent()) {
            Event.getCurrentEvent().initMap(this);
        }
    }
            ...
            if (p.x != x || p.y != y) {
                ...
                if (p.classId == 2) {
                    if (p.isHide) {
                        ...
                        Effect eff = em.findByType(EffectTypeName.TRANG_THAI_AN_THAN);
                        if (eff != null) {
    ...
    public Mob findMobLiveByID(int id) {
    ...
    public void join(Char p) {
        ...
        // =============Phân thân vào map cùng chủ thân (đi theo)===========================
        if (p.clone != null && p.clone.isNhanBan && !p.clone.isDead) {
    ...
    public void createMonster() {
        int id = 0;
        boolean isMapTask = this.id == 0 && tilemap.isNormal() && !tilemap.isThatThuAi() && !tilemap.isDauTruong() && !tilemap.isCandyBattlefield()
                && !tilemap.isDungeo() && !tilemap.isDungeoClan() && !tilemap.isLangTruyenThuyet() && !tilemap.isLangCo() && !tilemap.isNotSave()
