...
public class NymozCave extends AreaWithCountdownTime {
    ...
    public Bot createBoss(int id, String name, short mask, short head, short body, short leg, short x, short y) {
        Bot bot = Bot.builder().id(id).name(name).level(99).typePk(Char.PK_DOSAT).classId((byte) 0).build();
        bot.setDefault();
        FashionCustom fashionCustom = FashionCustom.builder().head(head).body(body).leg(leg).fMask(mask).build();
        bot.setFashionStrategy(fashionCustom);
        AbilityCustom abilityCustom = AbilityCustom.builder().hp(80000000) // 80000000
                .mp(10000000).damage(50000).damage2(50000).exactly(1000).fatal(200).miss(300).build();
