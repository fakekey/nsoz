...
public class MapService extends AbsService {
    ...
    public void moveFastNpc(Mob mob, Char _char) {
        try {
            Message mss = new Message(CMD.MOVE_FAST_NPC);
            DataOutputStream ds = mss.writer();
            ds.writeByte(mob.id);
            ds.writeInt(_char.id);
            ds.flush();
    ...
    public void moveFast(Char me, Char pl) {
        try {
            Message mss = new Message(CMD.MOVE_FAST);
            DataOutputStream ds = mss.writer();
            ds.writeInt(me.id);
            ds.writeShort(pl.x);
            ds.writeShort(pl.y);
    ...
    public void mobMeAttack(Char _c, Object target, Mob mobMe) {
        try {
            int mobID = -1;
            int targetID = -1;
            byte type = 0;
            ...
            ds.writeInt(_c.id);
            ds.writeByte(mobID);
            ds.writeShort(mobMe.idSkill_atk);
            ds.writeByte(NinjaUtils.nextInt(0, mobMe.template.frameBossAttack.length - 1)); // typeAtk
            ds.writeByte(mobMe.typeTool);
            if (type == 1) {
                ds.writeByte(type);
