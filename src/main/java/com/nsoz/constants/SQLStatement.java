...
public interface SQLStatement {
    ...
    public static final String SAVE_DATA_PLAYER =
            "UPDATE `players` SET `xu` = ?, `xuInBox` = ?, `yen` = ?, `point` = ?, `spoint` = ?, `saveCoordinate` = ?, `numberCellBag` = ?, `numberCellBox` = ?, `last_logout_time` = ?, `clan` = ?, `taskId` = ?, `rewardPB` = ?, `message` = ?, `data` = ?, `skill` = ?, `potential` = ?, `map` = ?, `equiped` = ?, `bag` = ?, `box` = ?, `mount` = ?, `task` = ?, `fashion` = ?, `class` = ?, `effect` = ?, `friends` = ?, `head2` = ?, `weapon` = ?, `body` = ?, `leg` = ?, `enemies` = ?, `onCSkill` = ?, `onKSkill` = ?, `onOSkill` = ?, `mask_box` = ?, `collection_box` = ?, `bijuu` = ?, `online` = ? WHERE `id` = ? LIMIT 1";
    public static final String GET_GIFT_CODE =
            "SELECT * FROM `gift_codes` WHERE `code` = ? AND (`server_id` = ? OR `server_id` = 0) AND (expires_at IS NULL OR expires_at > now()) LIMIT 1;";
    public static final String UPDATE_GIFT_CODE = "UPDATE `gift_codes` SET `status` = 1, `updated_at` = ? WHERE `id` = ? LIMIT 1;";
    public static final String INSERT_USED_GIFT_CODE =
            "INSERT INTO `gift_code_histories`(`player_id`,`user_id`, `gift_code`, `updated_at`) VALUES (?, ?, ?, ?)";
    public static final String CHECK_EXIST_USED_GIFT_CODE =
            "SELECT * FROM `gift_code_histories` WHERE `gift_code` = ? AND (`player_id` = ? OR `user_id` = ?) LIMIT 1;";
    public static final String GET_USER =
            "SELECT `status`, `id`, `luong`, `last_attendance_at`, `received_first_gift`, `password`, `ip_address`, `level_reward`, `role`, `online`, `ban_until`, `activated` FROM `users` WHERE `username` = ?;";
    ...
    public static final String CHECK_CHARNAME = "SELECT * FROM `players` WHERE `name` = ? LIMIT 1;";
    ...
    public static final String INSERT_ITEM_TO_STALL =
            "INSERT INTO `shinwa`(`server_id`, `seller`, `item`, `price`, `status`, `time`) VALUES (?,?,?,?,?,?)";
    ...
    public static final String LOAD_EVENT_POINT =
            "SELECT `event_points`.*, `players`.`name` FROM `event_points`, `players` WHERE `event_points`.`event_id` = ? AND `event_points`.`server_id` = ? AND `players`.`id` = `event_points`.`player_id`;";
