...
public class Events {
    ...
    public static void gioHoa(Char p, int number) {
        ...
        if (indexHoaHongDo == -1 || p.bag[indexHoaHongDo] == null || p.bag[indexHoaHongDo].getQuantity() < hoaHongDo) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Hoa hồng đỏ");
            return;
        }
        if (indexHoaHongVang == -1 || p.bag[indexHoaHongVang] == null || p.bag[indexHoaHongVang].getQuantity() < hoaHongVang) {
        ...
        if (indexHoaHongXanh == -1 || p.bag[indexHoaHongXanh] == null || p.bag[indexHoaHongXanh].getQuantity() < hoaHongXanh) {
    ...
    /*
     * public static void dieuVai(Char p, int number) { if (number < 1) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Số lượng tối thiểu là 1 cái."); return; } if (number >
     * 1000) { p.getService().npcChat(NpcName.TIEN_NU, "Chỉ có thể làm tối đa 1.000 cái."); return; }
     * int tre = 3 * number; int day = 3 * number; int giay = 3 * number; int gold = 15 * number; int
     * indexTre = p.getIndexItemByIdInBag(ItemName.TRE); int indexDay =
     * p.getIndexItemByIdInBag(ItemName.DAY); int indexGiay = p.getIndexItemByIdInBag(ItemName.GIAY2);
     ...
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Tre"); return; } if (indexDay == -1 ||
     * p.bag[indexDay] == null || p.bag[indexDay].getQuantity() < day) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Dây"); return; } if (indexGiay == -1 ||
     * p.bag[indexGiay] == null || p.bag[indexGiay].getQuantity() < giay) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Giấy"); return; } if (p.gold < gold) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ xu"); return; } if (p.getSlotNull() == 0) {
     * p.getService().npcChat(NpcName.TIEN_NU, p.language.getString("BAG_FULL")); return; }
     * p.removeItem(indexTre, tre, true); p.removeItem(indexDay, day, true); p.removeItem(indexGiay,
     * giay, true); Item item = ItemFactory.getInstance().newItem(ItemName.DIEU_VAI); item.isLock =
     * false; p.addGold(-gold);
     * 
     * item.setQuantity(number); p.addItemToBag(item); }
     * 
     * public static void dieuGiay(Char p, int number) { if (number < 1) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Số lượng tối thiểu là 1 cái."); return; } if (number >
     * 1000) { p.getService().npcChat(NpcName.TIEN_NU, "Chỉ có thể làm tối đa 1.000 cái."); return; }
     * int tre = 3 * number; int day = 3 * number; int giay = 3 * number; int coin = 300000 * number;
     * int indexTre = p.getIndexItemByIdInBag(ItemName.TRE); int indexDay =
     * p.getIndexItemByIdInBag(ItemName.DAY); int indexGiay = p.getIndexItemByIdInBag(ItemName.GIAY2);
     * int indexMauVe = p.getIndexItemByIdInBag(ItemName.MAU_VE_CAO_CAP);
     * 
     * if (indexTre == -1 || p.bag[indexTre] == null || p.bag[indexTre].getQuantity() < tre) {
     ...
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Dây"); return; } if (indexGiay == -1 ||
     * p.bag[indexGiay] == null || p.bag[indexGiay].getQuantity() < giay) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Giấy"); return; } if (p.coin < coin) {
     * p.getService().npcChat(NpcName.TIEN_NU, "Không đủ xu"); return; } if (p.getSlotNull() == 0) {
     * p.getService().npcChat(NpcName.TIEN_NU, p.language.getString("BAG_FULL")); return; }
     * p.removeItem(indexTre, tre, true); p.removeItem(indexDay, day, true); p.removeItem(indexGiay,
     * giay, true); Item item = ItemFactory.getInstance().newItem(ItemName.DIEU_GIAY); item.isLock =
     * false; item.setQuantity(number); p.addItemToBag(item); p.addCoin(-coin);
     * 
     * }
     */
    // sk Halloween
    public static void chocolate(Char p, int number) {
        ...
        if (indexChocolate == -1 || p.bag[indexChocolate] == null || p.bag[indexChocolate].getQuantity() < chocolate) {
    ...
    public static void dayPhao(Char p, int number) {
        ...
        if (indexManhPhao == -1 || p.bag[indexManhPhao] == null || p.bag[indexManhPhao].getQuantity() < manhPhaoHoa) {
    ...
    public static void hopQua(Char p, int number) {
        ...
        if (indexChuongVang == -1 || p.bag[indexChuongVang] == null || p.bag[indexChuongVang].getQuantity() < chuongVang) {
    ...
    public static void hopBanhThuong(Char p, int number) {
        ...
        if (indexBanhThapCam == -1 || p.bag[indexBanhThapCam] == null || p.bag[indexBanhThapCam].getQuantity() < banhThapCam) {
        ...
        if (indexBanhDauXanh == -1 || p.bag[indexBanhDauXanh] == null || p.bag[indexBanhDauXanh].getQuantity() < banhDauXanh) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Bánh đậu xanh");
            return;
        }
        if (indexGiayGoiThuong == -1 || p.bag[indexGiayGoiThuong] == null || p.bag[indexGiayGoiThuong].getQuantity() < giayGoiThuong) {
    ...
    public static void hopBanhThuongHang(Char p, int number) {
        ...
        if (indexGiayGoiCaoCap == -1 || p.bag[indexGiayGoiCaoCap] == null || p.bag[indexGiayGoiCaoCap].getQuantity() < giayGoiCaoCap) {
    ...
    public static void tiepTeLuongThuc(Char p, int itemId, int number) {
        ...
        if (itemId == ItemName.THAU_RAU_CU && (indexCaChua == -1 || p.bag[indexCaChua] == null || p.bag[indexCaChua].getQuantity() < amount)) {
            ...
            return;
        } else if (itemId == ItemName.THAU_THIT_CA
                && (indexTangThit == -1 || p.bag[indexTangThit] == null || p.bag[indexTangThit].getQuantity() < amount)) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ tảng thịt");
            return;
        } else if (itemId == ItemName.THAU_THIT_CA
                && (indexKhucCa == -1 || p.bag[indexKhucCa] == null || p.bag[indexKhucCa].getQuantity() < amount)) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ tảng thịt");
            return;
    ...
    public static void doiCup(Char p, int type) {
        ...
        if (p.getEventPoint().getPoint(EventPoint.DIEM_TIEU_XAI) < point) {
            p.getService().npcChat(NpcName.TIEN_NU,
                    "Ngươi cần tối thiểu " + NinjaUtils.getCurrency(point) + " điểm thi đấu mới có thể đổi được chiếc cúp này.");
            return;
        }
    ...
    // Mob monster = new Mob(z.getMonsters().size(), (short) template.id,
    // template.hp, template.level, (short) 1928, (short) 240, false,
    // template.isBoss(), z);
    // z.addMob(monster);
    // mouseMob = monster;
    //
