...
public class Server {
    ...
    public static boolean init() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                TaskTemplate task = TaskTemplate.builder().taskId(taskId).name(name).detail(detail).subNames(subnames).counts(counts)
                        .leveRequire(levelRequire).mobs(mobs).items(items).build();
    ...
    public static void start() {
        try {
            ...
            AutoMaintenance.maintenance(3, 55, 0);
    ...
    public static void maintance() {
        try {
            ...
            String text =
                    "Máy chủ bảo trì sau 5 phút, vui lòng thoát game để tránh mất dữ liệu. Nếu cố tình không thoát chúng tôi không chịu trách nhiệm!";
            GlobalService.getInstance().chat(name, text);
            GlobalService.getInstance().showAlert(name, text);
            Log.info("Hệ thống Đóng sau 5 phút.");
            Thread.sleep(240000);
            String text2 =
                    "Máy chủ bảo trì sau 1 phút, vui lòng thoát game để tránh mất dữ liệu. Nếu cố tình không thoát chúng tôi không chịu trách nhiệm!";
