...
public class ServerManager {
    ...
    private static ReadWriteLock lockBot = new ReentrantReadWriteLock();
    ...
    public static Bot findBotById(int id) {
        lockBot.readLock().lock();
        try {
            for (Bot _bot : bots) {
                if (_bot.id == id) {
                    return _bot;
            ...
            lockBot.readLock().unlock();
        }

        return null;
    }

    public static void removeBot(Bot _bot) {
        lockBot.writeLock().lock();
        try {
            bots.removeIf(b -> {
                if (b.id == _bot.id) {
                    return true;
            ...
            lockBot.writeLock().unlock();
        }
    }

    public static void putBot(Bot _bot) {
        lockBot.writeLock().lock();
        try {
            removeBot(_bot);
            bots.add(_bot);
